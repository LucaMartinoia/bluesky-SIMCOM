...
settings.set_variable_defaults(
    text_size=13, ac_size=16, asas_vmin=200.0, asas_vmax=500.0
)
...
class Traffic(glh.RenderObject, layer=100):
    ...
    def filteralt(self, flag: bool = None, bottom: "alt" = -1e99, top: "alt" = 1e99):
        ...
        if flag is None:
            if not self.altrange:
                ...
                return (
                    True,
                    f"The current altitude range is limited below {self.altrange[1]} meters",
                )
            elif self.altrange[1] > 1e90:
                return (
                    True,
                    f"The current altitude range is limited above {self.altrange[0]} meters",
                )
            else:
                return (
                    True,
                    f"The current altitude range is limited between {self.altrange[0]} and {self.altrange[1]} meters",
                )
        if flag:
    ...
    def create(self):
        ...
        self.ssd.create(
            lat1=self.lat, lon1=self.lon, alt1=self.alt, tas1=self.tas, trk1=self.hdg
        )
        self.ssd.set_attribs(
            selssd=MAX_NAIRCRAFT,
            instance_divisor=1,
            datatype=glh.gl.GL_UNSIGNED_BYTE,
            normalize=True,
        )
        self.ssd.set_attribs(
            lat0=self.lat,
        ...
        self.cpalines.create(
            ...
            usage=glh.GLBuffer.UsagePattern.StreamDraw,
        )
        ...
        self.route.create(
            vertex=ROUTE_SIZE * 8,
            color=palette.route,
            usage=glh.GLBuffer.UsagePattern.DynamicDraw,
        )

        self.routelbl.create(
            ROUTE_SIZE * 24,
            ROUTE_SIZE * 4,
            ROUTE_SIZE * 4,
            palette.route,
            (wpt_size, 0.5 * wpt_size),
            instanced=True,
        )
        rwptvertices = np.array(
            [
        ...
        self.rwaypoints.set_attribs(
            lat=self.routelbl.lat, lon=self.routelbl.lon, instance_divisor=1
        )
    ...
    def draw(self):
        ...
        if self.ssd_all or self.ssd_conflicts or len(self.ssd_ownship) > 0:
            ...
            ssd_shader.bind()
            glh.gl.glUniform3f(
                ssd_shader.uniforms["Vlimits"].loc,
                self.asas_vmin**2,
                self.asas_vmax**2,
                self.asas_vmax,
            )
    ...
    def update_route_data(self, data):
        ...
        if data.acid != "" and len(data.wplat) > 0:
            ...
            self.routelbl.update(
                texdepth=np.array(wpname.encode("ascii", "ignore")),
    ...
    def update_aircraft_data(self, data):
        ...
        if naircraft == 0:
            ...
            for i, (
                ...
                if self.show_lbl >= 1:
                    rawlabel += "%-8s" % acid[:8]
                    if self.show_lbl == 2 and (not np.isnan(alt)):
                        if alt <= data.translvl:
                            rawlabel += "%-5d" % int(alt / ft + 0.5)
                        else:
                            rawlabel += "FL%03d" % int(alt / ft / 100.0 + 0.5)
