...
if platform.system().lower() == "windows":
    ...
    def isdark():
        ...
        return (
            p.color(p.ColorRole.Window).value()
            < p.color(p.ColorRole.WindowText).value()
        )
...
palette.set_default_colours(stack_text=(0, 255, 0), stack_background=(102, 102, 102))
...
class Splash(QSplashScreen):
    ...
    def __init__(self):
        ...
        super().__init__(
            QPixmap(splashfile.as_posix()), Qt.WindowType.WindowStaysOnTopHint
        )
...
class MainWindow(QMainWindow, Base):
    ...
    def __init__(self, mode):
        ...
        if isinstance(scenresource, ResourcePath) and scenresource.nbases > 1:
            ...
            openpkg.triggered.connect(
                lambda: self.show_file_dialog(scenresource.base(-1))
            )
            openusr = openfrom.addAction("User")
            openusr.triggered.connect(
                lambda: self.show_file_dialog(scenresource.base(0))
            )
    ...
    def setStyleSheet(self, contents=""):
        if not contents:
            QDir.addSearchPath(
                "icons", (bs.resource(bs.settings.gfx_path) / "icons").as_posix()
            )
    ...
    def mcre(self, args: "string"):
        ...
        stack.forward(
            f'INSIDE {" ".join(str(el) for el in bs.ref.area.bbox)} MCRE {args}'
        )

    @stack.command(
        annotations="pandir/latlon",
        brief="PAN latlon/acid/airport/waypoint/LEFT/RIGHT/UP/DOWN",
    )
    def pan(self, *args):
    ...
    @stack.command(
        name="QUIT", annotations="", aliases=("CLOSE", "END", "EXIT", "Q", "STOP")
    )
    ...
    def actnodedataChanged(self, nodeid, nodedata, changed_elems):
        if nodeid != self.actnode:
            ...
            self.nodetree.setCurrentItem(
                node, 0, QItemSelectionModel.SelectionFlag.ClearAndSelect
            )
    ...
    def serversChanged(self, server_id):
        ...
        if not server:
            ...
            if server_id != b"0":
                ...
                self.nodetree.setItemWidget(
                    server, 0, QLabel(hostname, parent=self.nodetree)
                )
    ...
    def on_siminfo_received(self, speed, simdt, simt, simutc, ntraf, state, scenname):
        ...
        if ctx.sender_id == bs.net.act_id:
            self.siminfoLabel.setText(
                "<b>t:</b> %s, <b>\u0394t:</b> %.2f, <b>Speed:</b> %.1fx, <b>UTC:</b> %s, <b>Mode:</b> %s, <b>Aircraft:</b> %d, <b>Conflicts:</b> %d/%d, <b>LoS:</b> %d/%d"
                % (
                    simt,
                    simdt,
                    speed,
        ...
        if platform.system().lower() == "windows":
            ...
            if platform.system().lower() == "darwin":
                response = QFileDialog.getOpenFileName(
                    self, "Open file", scenpath, "Scenario files (*.scn)"
                )
            else:
                response = QFileDialog.getOpenFileName(
                    self,
