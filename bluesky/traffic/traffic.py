...
class Traffic(Entity):
    ...
    def cre(
        self, acid, actype="B744", aclat=52.0, aclon=4.0, achdg=None, acalt=0, acspd=0
    ):
        ...
        if self.wind.winddim > 0:
            ...
            self.trk[-n:] = np.logical_not(applywind) * achdg + applywind * np.degrees(
    ...
    def update_airspeed(self):
        ...
        # Update velocities
        self.tas = np.where(
            need_ax, self.tas + self.ax * bs.sim.simdt, self.aporasas.tas
        )
        ...
        self.swaltsel = np.abs(delta_alt) > 1.05 * np.maximum(
            np.abs(bs.sim.simdt * self.aporasas.vs), np.abs(bs.sim.simdt * self.vs)
        )
    ...
    def poscommand(self, idxorwp: int | str):
        ...
        if isinstance(idxorwp, str):
            ...
            if iap >= 0:
                ...
                lines = (
                    lines
                    + bs.navdb.aptname[iap]
                    + "\n"
                    + "is a "
                    + aptypes[max(-1, bs.navdb.aptype[iap] - 1)]
                    + " airport at:\n"
            ...
            # Not found as airport, try waypoints & navaids
                ...
                if iwps[0] >= 0:
                    ...
                    for i in iwps:
                        ...
                        # Navaid: frequency
                        if (
                            bs.navdb.wptype[i] in ["VOR", "DME", "TACAN"]
                            and not samedesc
                        ):
                    ...
                    if bs.navdb.wptype[iwp] == "VOR":
                        lines = (
                            lines + "\nVariation: " + str(bs.navdb.wpvar[iwp]) + " deg"
                        )
                    ...
                    if nother > 0:
                        ...
                        lines = (
                            ...
                            + verb
                            + str(nother)
                            + " other waypoint(s) also named "
                            + wp
                        )

                    # In which airways?
                    connect = bs.navdb.listconnections(
                        wp, bs.navdb.wplat[iwp], bs.navdb.wplon[iwp]
                    )
                    if len(connect) > 0:
                        awset = set([])
                        for c in connect:
                ...
                # Try airway id
                    ...
                    if len(airway) > 0:
                        ...
                        return (
                            False,
                            idxorwp + " not found as a/c, airport, navaid or waypoint",
                        )
