...
class ADSBRadar(RenderObject, layer=101):
    ...
    def update_adsb_data(self, data):
        ...
        self.lat.update(np.array(lat, dtype=np.float32))
        self.lon.update(np.array(lon, dtype=np.float32))
        self.hdg.update(np.array(track, dtype=np.float32))
        self.alt.update(np.array(alt, dtype=np.float32))

        # Update color depending on danger logic
        color = np.tile(green_clr, (min(MAX_NAIRCRAFT, self.naircraft), 1))
        if self.show_danger:
