...
class ADSBprotocol(core.Entity):
    ...
    def update(self):
        ...
        # TO DO: add if-check for attacks
        noise = np.random.uniform(-150, 150, size=traf.ntraf)
        GNSSalt = traf.alt + noise
        self.ADSBaltGNSS = np.maximum(GNSSalt, 0)
        self.ADSBaltBaro = traf.alt
        self.ADSBlat     = traf.lat
        self.ADSBlon     = traf.lon
        self.ADSBtas     = traf.tas
        self.ADSBgsnorth = traf.gsnorth
        self.ADSBgseast  = traf.gseast
        self.ADSBvs      = traf.vs
        self.ADSBhdg     = traf.hdg
        self.ADSBtrk     = traf.trk
