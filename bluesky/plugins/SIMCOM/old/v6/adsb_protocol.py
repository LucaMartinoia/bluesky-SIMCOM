...
class ADSBprotocol(core.Entity):
    ...
    def __init__(self):
        ...
        with traf.settrafarrays():
            ...
            traf.ADSBgs = np.array([])  # ground speed [m/s]
    ...
    @stack.command(name="ADSBLOG", brief="ADSBLOG dt")
    def ADSBlogger(self, dt: int = 1):
        """Initialize the ADS-B data logging."""

        header = r"SIMCOM_DATA_LOG"
        data = (
            "traf.id, traf.ADSBattack, traf.lat, traf.lon, traf.alt, traf.gsnorth, traf.gseast, traf.hdg,"
            "traf.ADSBlat, traf.ADSBlon, traf.ADSBaltBaro, traf.ADSBgsnorth, traf.ADSBgseast, traf.ADSBhdg, traf.ADSBcallsign,"
            "traf.ADSBmsg_pos_o, traf.ADSBmsg_pos_e, traf.ADSBmsg_v, traf.ADSBmsg_id"
        )
        stack.stack(f"CRELOG SIMCOM {dt}, {header}")
        stack.stack(f"SIMCOM ADD {data}")
        stack.stack("SIMCOM ON")
        return True, f"Data logging initialized. SIMCOM OFF to stop the process."
