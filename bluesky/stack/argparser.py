...
from bluesky.tools.misc import (
    txt2bool,
    txt2lat,
    txt2lon,
    txt2alt,
...
class WpinrouteArg(Parser):
    ...
    def parse(self, argstring):
        ...
        if (
            bs.ref.acidx >= 0
            and wpname in bs.traf.ap.route[bs.ref.acidx].wpname
            or wpname == "*"
        ):
            return wpname, argstring
        raise ArgumentError(
            f"{wpname} not found in the route of {bs.traf.id[bs.ref.acidx]}"
        )
